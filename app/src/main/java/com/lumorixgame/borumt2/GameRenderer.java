...
public class GameRenderer implements GLSurfaceView.Renderer {
 float[] p=new float[16],v=new float[16],m=new float[16],mv=new float[16]; float yaw=0,pitch=.12f; Player player=new Player(); int prog,pos,col,mat;
 FloatBuffer ground;
 ...
 public void onDrawFrame(GL10 g){GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT|GLES20.GL_DEPTH_BUFFER_BIT);float cx=(float)Math.sin(yaw)*7,cz=(float)Math.cos(yaw)*7,cy=3.2f+pitch*2;Matrix.setLookAtM(v,0,player.x+cx,cy,player.z+cz,player.x,1,player.z,0,1,0);cube(player.x,.75f,player.z,.55f,.75f,.35f,.75f,.55f,.35f);cube(player.x,1.75f,player.z,.45f,.45f,.45f,.75f,.62f,.42f);cube(-3,.7f,-3,.5f,.7f,.35f,.55f,.45f,.72f);cube(3,.7f,-4,.5f,.7f,.35f,.55f,.45f,.72f);ground();}
